...
export default function Hero({ onOpenWaitlist }) {
  ...
  const layer2 = useTransform(scrollY, [0, 600], [0, 120]);
  const layer3 = useTransform(scrollY, [0, 600], [0, 180]);

  return (
    <section className="relative h-[95vh] w-full overflow-hidden bg-[#0E0E0E] text-white">
      {/* 3D Spline scene */}
      <div className="absolute inset-0">
        <Spline scene="https://prod.spline.design/4Zh-Q6DWWp5yPnQf/scene.splinecode" style={{ width: '100%', height: '100%' }} />
      </div>

      {/* Vectors & gradient glows with parallax */}
      ...
      <div className="relative z-10 mx-auto flex h-full max-w-6xl flex-col items-center justify-center px-6 text-center">
        ...
        <motion.p
          ...
          className="mx-auto mb-8 max-w-2xl text-lg text-white/85 sm:text-xl"
        >
        ...
        <motion.button
          ...
          animate={{
            boxShadow: [
              ...
              '0 0 0 14px rgba(59,130,246,0.12)',
              '0 0 0 0 rgba(59,130,246,0.0)'
            ],
          }}
          transition={{ repeat: Infinity, duration: 2.3 }}
          className="group relative rounded-xl bg-blue-500 px-6 py-3 text-base font-semibold text-black shadow-blue-500/30 sm:text-lg"
        >
          <span className="relative z-10">Join the Waitlist</span>
          <span className="absolute inset-0 rounded-xl bg-gradient-to-b from-white/30 to-transparent opacity-0 transition-opacity group-hover:opacity-100" />
        </motion.button>
      </div>
    </section>
  );
...
function Badge({ label, color = 'emerald' }) {
  const map = {
    emerald: { dot: 'bg-emerald-400', border: 'border-emerald-300/30' },
    blue: { dot: 'bg-blue-400', border: 'border-blue-300/30' },
  };
  ...
  return (
    <motion.div
      ...
      className={`flex items-center gap-2 rounded-2xl border ${c.border} bg-white/[0.06] px-3 py-2 backdrop-blur`}
    >
